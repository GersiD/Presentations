...
\frametitle{Imitation Learning}
	...
	\[ 
		\min_{\theta} \sum_{(s,a)}^{D_e} \mathcal{L} \left(\pi_\theta(s) - a \right)
	\]
	\vfill
	\textbf{Benifits}
	\begin{itemize}
...
\begin{frame} \frametitle{Markov Decision Process}
  \textbf{Model} (tabular in this talk) \par
   ...
   ~~~Features $\Phi \in \Real^{\mathcal{S} \times \mathcal{A} \times k}$ \par
...
\begin{frame}
	\frametitle{Realizeable Rewards}
...
\[ \begin{mprog}
	...
	\stc t \geq \max_{u_e \in \Upsilon} u_e\tr r - u\tr r,\quad \forall \; r \in ext(\Psi(\mathcal{R})),
        \cs u\in \Upsilon
\end{mprog} \]

\begin{itemize}
	\item $\Psi(\mathcal{R}) = \lbrace r \in \mathcal{R} \mid \hat{u}_e\tr r >= 0 \rbrace $
\end{itemize}
